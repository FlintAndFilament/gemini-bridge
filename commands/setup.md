...
allowed-tools: Bash(uv:*), Bash(UV_PROJECT_ENVIRONMENT=*), Bash(curl:*), Bash(sh:*), Bash(powershell:*), AskUserQuestion
---
...
instead of a second one:
    ...
    UV_PROJECT_ENVIRONMENT="${CLAUDE_PLUGIN_DATA}/venv" uv run --frozen --quiet --directory "${CLAUDE_PLUGIN_ROOT}" gemini-bridge-setup <subcommand> …
...
1. Run `uv --version`. If uv is missing, use AskUserQuestion once: "Install uv
   ...
   If the user skips, print the command for their OS and stop.
2. Run:

       UV_PROJECT_ENVIRONMENT="${CLAUDE_PLUGIN_DATA}/venv" uv run --frozen --quiet --directory "${CLAUDE_PLUGIN_ROOT}" gemini-bridge-setup status
   ...
   value — and ask them for the variable name instead.
3. Ask with AskUserQuestion, offering the saved value from `values` as the
   ...
   - Default thinking level (`none`/`low`/`medium`/`high`, default
     `values.default_thinking`) and default model (blank = newest Flash; `-`
     clears a saved value).
4. Run write with only the answers actually given, quoting every value in
   single quotes:

       UV_PROJECT_ENVIRONMENT="${CLAUDE_PLUGIN_DATA}/venv" uv run --frozen --quiet --directory "${CLAUDE_PLUGIN_ROOT}" gemini-bridge-setup write --auth-method '…' [--project '…'] [--location '…'] [--thinking '…'] [--default-model '…'] [--transcript-dir '…'] [--keychain-service '…'] [--keychain-account '…'] [--api-key-env '…']
